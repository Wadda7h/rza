...
function getUserRankProgress(userPosts) {
    ...
    const ranks = [
        ...
        { name: "أسطورة Z-Zone", icon: "diamond", threshold: 5000 }
    ];
    ...
    let tooltipText = "لقد وصلت إلى أعلى رتبة! أسطورة Z-Zone";
        ...
        try {
            ...
            for (let m of members) {
                ...
                if (match && match[1] && match[1].trim() !== '') {
                    ...
                    if (dateNode) {
                        let dateMatch = dateNode.textContent.match(/\d{1,4}[-/]\d{1,2}[-/]\d{1,4}/);
...
$(window).scroll(debounce(function() {
    if ($(this).scrollTop() > 300) {
        if ($('#back-to-top').length === 0) {
            $('body').append('<div id="back-to-top" style="position:fixed; bottom:20px; left:20px; background:var(--primary); color:#fff; width:45px; height:45px; border-radius:50%; display:flex; align-items:center; justify-content:center; cursor:pointer; z-index:9999; box-shadow:0 4px 12px var(--shadow-color); transition: background 0.3s, transform 0.3s;"><i class="material-symbols-outlined">arrow_upward</i></div>');
